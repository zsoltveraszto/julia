...
function (*)(J::UniformScaling, A::AbstractVecOrMat)
    if J.λ == 1
        return A
    else
        return J.λ * A
...
function (/)(J1::UniformScaling, J2::UniformScaling)
    if J2.λ == 0
        throw(SingularException(1))
    end
    return UniformScaling(J1.λ / J2.λ)
end
...
function (\){T<:Number}(A::Union(Bidiagonal{T},AbstractTriangular{T}),
                        J::UniformScaling)
    return inv(A) * J.λ
end
