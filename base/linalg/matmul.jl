...
scale(A::Matrix, b::Vector) =
    scale!(similar(A, promote_type(eltype(A), eltype(b))), A, b)

scale(b::Vector, A::Matrix) =
    scale!(similar(b, promote_type(eltype(A), eltype(b)), size(A)), b, A)
...
function dot{T<:BlasReal, TI<:Integer}(x::Vector{T},
...
function dot{T<:BlasComplex, TI<:Integer}(x::Vector{T},
                                          rx::Union(UnitRange{TI},Range{TI}),
                                          y::Vector{T},
                                          ry::Union(UnitRange{TI},Range{TI}))
    if length(rx) != length(ry)
...
At_mul_B{T<:BlasComplex}(x::StridedVector{T}, y::StridedVector{T}) =
    [BLAS.dotu(x, y)]

# Matrix-vector multiplication
function (*){T<:BlasFloat,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return A_mul_B!(similar(x, TS, size(A,1)), A, convert(AbstractVector{TS}, x))
end

function (*){T,S}(A::AbstractMatrix{T}, x::AbstractVector{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return A_mul_B!(similar(x, TS, size(A,1)), A, x)
end

(*)(A::AbstractVector, B::AbstractMatrix) = reshape(A,length(A),1)*B

A_mul_B!{T<:BlasFloat}(y::StridedVector{T}, A::StridedVecOrMat{T}, x::StridedVector{T}) =
    gemv!(y, 'N', A, x)

function A_mul_B!{T<:Union(Float32,Float64)}(y::StridedVector{Complex{T}},
                                             A::StridedVecOrMat{Complex{T}},
                                             x::StridedVector{T})
    Afl = reinterpret(T, A, (2*size(A,1), size(A,2)))
    yfl = reinterpret(T,y)
    gemv!(yfl,'N',Afl,x)
    return y
end

A_mul_B!(y::StridedVector, A::StridedVecOrMat, x::StridedVector) =
    generic_matvecmul!(y, 'N', A, x)

function At_mul_B{T<:BlasFloat,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return At_mul_B!(similar(x, TS, size(A,2)), A, convert(AbstractVector{TS}, x))
end

function At_mul_B{T,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return At_mul_B!(similar(x, TS, size(A,2)), A, x)
end

function At_mul_B!{T<:BlasFloat}(y::StridedVector{T},
                                 A::StridedVecOrMat{T},
                                 x::StridedVector{T})
    return gemv!(y, 'T', A, x)
end

At_mul_B!(y::StridedVector, A::StridedVecOrMat, x::StridedVector) =
    generic_matvecmul!(y, 'T', A, x)

function Ac_mul_B{T<:BlasFloat,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return Ac_mul_B!(similar(x, TS, size(A,2)), A, convert(AbstractVector{TS}, x))
end

function Ac_mul_B{T,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T),arithtype(S))
    return Ac_mul_B!(similar(x,TS,size(A,2)), A, x)
end

function Ac_mul_B!{T<:BlasReal}(y::StridedVector{T},
                                A::StridedVecOrMat{T},
                                x::StridedVector{T})
    return At_mul_B!(y, A, x)
end

function Ac_mul_B!{T<:BlasComplex}(y::StridedVector{T},
                                   A::StridedVecOrMat{T},
                                   x::StridedVector{T})
    return gemv!(y, 'C', A, x)
end

Ac_mul_B!(y::StridedVector, A::StridedVecOrMat, x::StridedVector) =
    generic_matvecmul!(y, 'C', A, x)

# Matrix-matrix multiplication

function (*){T,S}(A::AbstractMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return A_mul_B!(similar(B, TS, (size(A,1), size(B,2))), A, B)
end

function A_mul_B!{T<:BlasFloat}(C::StridedMatrix{T},
                                A::StridedVecOrMat{T},
                                B::StridedVecOrMat{T})
    return gemm_wrapper!(C, 'N', 'N', A, B)
end

function A_mul_B!{T<:Union(Float32,Float64)}(C::StridedMatrix{Complex{T}},
                                             A::StridedVecOrMat{Complex{T}},
                                             B::StridedVecOrMat{T})
    Afl = reinterpret(T, A, (2*size(A,1), size(A,2)))
    Cfl = reinterpret(T, C, (2*size(C,1), size(C,2)))
    gemm_wrapper!(Cfl, 'N', 'N', Afl, B)
    return C
end

A_mul_B!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'N', 'N', A, B)

function At_mul_B{T,S}(A::AbstractMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return At_mul_B!(similar(B, TS, (size(A,2), size(B,2))), A, B)
end

function At_mul_B!{T<:BlasFloat}(C::StridedMatrix{T},
                                 A::StridedVecOrMat{T},
                                 B::StridedVecOrMat{T})
    if is(A,B)
...
At_mul_B!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'T', 'N', A, B)

function A_mul_Bt{T,S}(A::AbstractMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return A_mul_Bt!(similar(B, TS, (size(A,1), size(B,1))), A, B)
end

function A_mul_Bt!{T<:BlasFloat}(C::StridedMatrix{T},
                                 A::StridedVecOrMat{T},
                                 B::StridedVecOrMat{T})
    if is(A,B)
...
function A_mul_Bt!{T<:Union(Float32,Float64)}(C::StridedMatrix{Complex{T}},
                                              A::StridedVecOrMat{Complex{T}},
                                              B::StridedVecOrMat{T})
    Afl = reinterpret(T, A, (2*size(A,1), size(A,2)))
    Cfl = reinterpret(T, C, (2*size(C,1), size(C,2)))
...
A_mul_Bt!(C::StridedVecOrMat, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'N', 'T', A, B)

function At_mul_Bt{T,S}(A::AbstractMatrix{T}, B::StridedVecOrMat{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return At_mul_Bt!(similar(B, TS, (size(A,2), size(B,1))), A, B)
end

function At_mul_Bt!{T<:BlasFloat}(C::StridedMatrix{T},
                                  A::StridedVecOrMat{T},
                                  B::StridedVecOrMat{T})
    return gemm_wrapper!(C, 'T', 'T', A, B)
end

At_mul_Bt!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'T', 'T', A, B)

Ac_mul_B{T<:BlasReal}(A::StridedMatrix{T}, B::StridedMatrix{T}) =
    At_mul_B(A, B)

function Ac_mul_B!{T<:BlasReal}(C::StridedMatrix{T},
                                A::StridedVecOrMat{T},
                                B::StridedVecOrMat{T})
    return At_mul_B!(C, A, B)
end

function Ac_mul_B{T,S}(A::StridedMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return Ac_mul_B!(similar(B, TS, (size(A,2), size(B,2))), A, B)
end

function Ac_mul_B!{T<:BlasComplex}(C::StridedMatrix{T},
                                   A::StridedVecOrMat{T},
                                   B::StridedVecOrMat{T})
    if is(A,B)
...
Ac_mul_B!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'C', 'N', A, B)

A_mul_Bc{T<:BlasFloat,S<:BlasReal}(A::StridedMatrix{T}, B::StridedMatrix{S}) =
    A_mul_Bt(A, B)

function A_mul_Bc!{T<:BlasFloat,S<:BlasReal}(C::StridedMatrix{T},
                                             A::StridedVecOrMat{T},
                                             B::StridedVecOrMat{S})
    return A_mul_Bt!(C, A, B)
end

function A_mul_Bc{T,S}(A::StridedMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return A_mul_Bc!(similar(B, TS, (size(A,1), size(B,1))), A, B)
end

function A_mul_Bc!{T<:BlasComplex}(C::StridedMatrix{T},
                                   A::StridedVecOrMat{T},
                                   B::StridedVecOrMat{T})
    if is(A,B)
...
A_mul_Bc!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'N', 'C', A, B)

function Ac_mul_Bc{T,S}(A::AbstractMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(T), arithtype(S))
    return Ac_mul_Bc!(similar(B, TS, (size(A,2), size(B,1))), A, B)
end

function Ac_mul_Bc!{T<:BlasFloat}(C::StridedMatrix{T},
                                  A::StridedVecOrMat{T},
                                  B::StridedVecOrMat{T})
    return gemm_wrapper!(C, 'C', 'C', A, B)
end

Ac_mul_Bc!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'C', 'C', A, B)

function Ac_mul_Bt{T,S}(A::AbstractMatrix{T}, B::StridedMatrix{S})
    TS = promote_type(arithtype(A), arithtype(B))
    return Ac_mul_Bt!(similar(B, TS, (size(A,2), size(B,1))), A, B)
end

Ac_mul_Bt!(C::StridedMatrix, A::StridedVecOrMat, B::StridedVecOrMat) =
    generic_matmatmul!(C, 'C', 'T', A, B)
...
function syrk_wrapper!{T<:BlasFloat}(C::StridedMatrix{T},
                                     tA::Char,
                                     A::StridedVecOrMat{T})
    nC = chksquare(C)
    if tA == 'T'
    ...
    if nC != mA
        throw(DimensionMismatch(
            "output matrix has size: $(nC), but should have size $(mA)"))
    end
    if mA == 0 || nA == 0
        return fill!(C,0)
    end
...
function herk_wrapper!{T<:BlasFloat}(C::StridedMatrix{T},
                                     tA::Char,
                                     A::StridedVecOrMat{T})
    nC = chksquare(C)
    if tA == 'C'
...
function gemm_wrapper{T<:BlasFloat}(tA::Char, tB::Char,
    ...
    C = similar(B, T, mA, nB)
    return gemm_wrapper!(C, tA, tB, A, B)
end

function gemm_wrapper!{T<:BlasFloat}(C::StridedVecOrMat{T},
                                     tA::Char, tB::Char,
                                     A::StridedVecOrMat{T},
                                     B::StridedVecOrMat{T})
    mA, nA = lapack_size(tA, A)
...
function lapack_size(t::Char, M::AbstractVecOrMat)
    mM, nM = size(M,1), size(M,2)
    if t == 'N'
        return (mM, nM)
    else
        return (nM, mM)
...
function copy!{R,S}(B::AbstractVecOrMat{R},
                    ir_dest::UnitRange{Int},
                    jr_dest::UnitRange{Int},
                    tM::Char,
                    M::AbstractVecOrMat{S},
                    ir_src::UnitRange{Int},
                    jr_src::UnitRange{Int})
    if tM == 'N'
...
function copy_transpose!{R,S}(B::AbstractMatrix{R},
                              ir_dest::UnitRange{Int},
                              jr_dest::UnitRange{Int},
                              tM::Char,
                              M::AbstractVecOrMat{S},
...
function generic_matvecmul!{T,S,R}(C::AbstractVector{R},
                                   tA::Any,
...
function generic_matmatmul{T,S}(tA::Any,
                                tB::Any,
                                A::AbstractVecOrMat{T},
                                B::AbstractMatrix{S})
    mA, nA = lapack_size(tA, A)
...
function generic_matmatmul!{T,S,R}(C::AbstractVecOrMat{R},
                                   tA::Any,
                                   tB::Any,
                                   A::AbstractVecOrMat{T},
...
matmul2x2{T,S}(tA, tB, A::AbstractMatrix{T}, B::AbstractMatrix{S}) =
...
function matmul2x2!{T,S,R}(C::AbstractMatrix{R}, tA, tB, A::AbstractMatrix{T}, B::AbstractMatrix{S})
    if tA == 'T'
        A11 = transpose(A[1,1]); A12 = transpose(A[2,1])
        A21 = transpose(A[1,2]); A22 = transpose(A[2,2])
    elseif tA == 'C'
        A11 = ctranspose(A[1,1]); A12 = ctranspose(A[2,1])
        A21 = ctranspose(A[1,2]); A22 = ctranspose(A[2,2])
    else
        A11 = A[1,1]; A12 = A[1,2]
        A21 = A[2,1]; A22 = A[2,2]
    end
    if tB == 'T'
        B11 = transpose(B[1,1]); B12 = transpose(B[2,1])
        B21 = transpose(B[1,2]); B22 = transpose(B[2,2])
    elseif tB == 'C'
        B11 = ctranspose(B[1,1]); B12 = ctranspose(B[2,1])
        B21 = ctranspose(B[1,2]); B22 = ctranspose(B[2,2])
    else
        B11 = B[1,1]; B12 = B[1,2]
        B21 = B[2,1]; B22 = B[2,2]
    end
...
matmul3x3{T,S}(tA, tB, A::AbstractMatrix{T}, B::AbstractMatrix{S}) =
