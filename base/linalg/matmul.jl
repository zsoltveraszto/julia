...
function A_mul_B!{T<:BlasFloat, S<:BlasFloat}(tA::Char, α::Number, A::StridedMatrix{T}, x::StridedVector{S}, β::Number, y::StridedVector)
    TS = promote_type(T,S)
    A_mul_B!(tA, convert(TS, α), convert(Matrix{TS}, A), convert(Vector{TS}, x), convert(TS, β), y)
end
...
function (*){T,S}(A::StridedVecOrMat{T}, x::StridedVector{S})
    ...
    A_mul_B!('N', one(TS), A, x, zero(TS), zeros(TS, size(A,1)))
end
function Ac_mul_B{T,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T),arithtype(S))
    A_mul_B!('C', one(TS), A, x, zero(TS), zeros(TS, size(A,2)))
end
function At_mul_B{T,S}(A::StridedMatrix{T}, x::StridedVector{S})
    TS = promote_type(arithtype(T),arithtype(S))
    A_mul_B!('T', one(TS), A, x, zero(TS), zeros(TS, size(A,2)))
end
...
function generic_matvecmul!{T,S,R}(tA, α::Number, A::StridedMatrix{T}, x::StridedVector{S}, β::Number, y::StridedVector{R})
    ...
    my = length(y)
    mA, nA = lapack_size(tA, A)
    nA == mx || throw(DimensionMismatch(""))
    my == mA || throw(DimensionMismatch("output size does not match"))
    z = zero(R)
...
function generic_matmatmul!{T,S,R}(tA::Char, tB::Char, α::Number, A::StridedVecOrMat{T}, B::StridedMatrix{S}, β::Number, C::StridedVecOrMat{R})
    mA, nA = lapack_size(tA, A)
    mB, nB = lapack_size(tB, B)
    nA == mB || throw(DimensionMismatch("Argument shapes do not match"))
    (size(C,1) == mA && size(C,2) == nB) || throw(DimensionMismatch("Output size is incorrect"))
