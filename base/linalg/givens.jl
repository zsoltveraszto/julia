transpose(R::AbstractRotation) =
    error("transpose not implemented for $(typeof(R)).",
          " Consider using conjugate transpose (') instead of transpose (.').")

function (*){T,S}(R::AbstractRotation{T}, A::AbstractMatrix{S})
    TS = typeof(zero(T)*zero(S) + zero(T)*zero(S))
    if TS == S
        return A_mul_B!(convert(AbstractRotation{TS}, R),
                        copy(A))
    else
        return A_mul_B!(convert(AbstractRotation{TS}, R),
...
function A_mul_Bc{T,S}(A::AbstractMatrix{T}, R::AbstractRotation{S})
    TS = typeof(zero(T)*zero(S) + zero(T)*zero(S))
    if TS == T
        return A_mul_Bc!(copy(A),
                         convert(AbstractRotation{TS}, R))
    else
        return A_mul_Bc!(convert(AbstractArray{TS}, A),
                         convert(AbstractRotation{TS}, R))
    end
end
...
immutable Rotation{T} <: AbstractRotation{T}
    rotations::Vector{Givens{T}}
end

convert{T}(::Type{Givens{T}}, G::Givens{T}) = G
convert{T}(::Type{Givens{T}}, G::Givens) =
    Givens(G.i1, G.i2, convert(T, G.c), convert(T, G.s))

convert{T}(::Type{Rotation{T}}, R::Rotation{T}) = R
convert{T}(::Type{Rotation{T}}, R::Rotation) =
    Rotation{T}([convert(Givens{T}, g) for g in R.rotations])
...
ctranspose{T}(R::Rotation{T}) =
    Rotation{T}(reverse!([ctranspose(r) for r in R.rotations]))

realmin2(::Type{Float32}) = reinterpret(Float32, 0x26000000)
realmin2(::Type{Float64}) = reinterpret(Float64, 0x21a0000000000000)

function realmin2{T}(::Type{T})
    twopar = 2 * one(T)
    return twopar ^ trunc(Integer, log(realmin(T)/eps(T)) / log(twopar)/twopar)
end

function givens_algorithm{T<:FloatingPoint}(f::T, g::T)
    zeropar = zero(T)
    onepar = one(T)
    twopar = 2one(T)
...
function givens_algorithm{T<:FloatingPoint}(f::Complex{T}, g::Complex{T})
...
function givens{T}(f::T, g::T, i1::Integer, i2::Integer)
    ...
    c, s, r = givens_algorithm(f, g)
    return (Givens(i1, i2, convert(T, c), convert(T, s)), r)
end
...
function givens{T}(A::AbstractMatrix{T}, i1::Integer, i2::Integer, col::Integer)
    ...
    c, s, r = givens_algorithm(A[i1,col], A[i2,col])
    return (Givens(i1, i2, convert(T, c), convert(T, s)), r)
end
...
(*){T}(G1::Givens{T}, G2::Givens{T}) =
    Rotation(push!(push!(Givens{T}[], G2), G1))
